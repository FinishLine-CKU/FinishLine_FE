# CKU 졸업요건 검사 서비스 Finish Line


<div align="center">

  <a href="https://finishline-cku.com">
    <img src="https://github.com/user-attachments/assets/c4fef475-027b-4027-92b7-c17050f19469" /> 
  </a>
  
  ### <a href="https://finishline-cku.com" target="_blank">Finish Line 바로가기</a>

</div>

<br>

## 🎓 프로젝트 소개

<div align="center">

  <img src="https://github.com/user-attachments/assets/331f0777-c8e1-4c2e-ad2d-c7e799dcd833" width="700px">

  ### 가톨릭관동대 졸업요건 검사 서비스 Finish Line

  <p>2018~2025학년도 <strong>(18~25학번)</strong>, 총 <strong>53개 학과/전공 재학생</strong>의 졸업 요건을 반영했습니다.</p>
  <p><strong>재학생 인증</strong>을 통해 서비스에 가입하고, 종합정보시스템 <strong>학기별 성적 조회 PDF</strong>를 첨부하여 기이수과목을 등록할 수 있습니다.</p>
  <p>검사 결과를 시각화하여 <strong>부족한 학점</strong>과 <strong>영역</strong>을 한눈에 파악할 수 있습니다.</p>
  
</div>

<br>

## 🔍 기능 소개
> 각 항목을 클릭하여 자세한 내용을 확인할 수 있습니다.

<details>
  <summary>
    
  #### [ 재학생 인증 ]
  
  </summary>

  | ![학생인증](https://github.com/user-attachments/assets/7bdb13c4-944e-4533-abdf-f0a2790cd305) |
  |:--:|
  | **재학생 인증 & 회원가입** |

  - 재학생 인증 완료 후 FinishLine에 회원가입 할 수 있습니다.
  - 가톨릭관동대 포털 **`아이디/비밀번호`** 를 입력하여 재학생 인증을 진행합니다.
  - 재학생 인증 후 **`기본 정보(성명, 학번, 학과/전공)`** 를 확인합니다.
  - **`추가 전공`** 과 **`소단위 전공`** 대상자는 일치하는 정보를 선택합니다.
  - **`비밀번호를 설정`** 하여 회원가입을 마무리합니다.
    
</details>

<details>
  <summary>
    
  #### [ 마이페이지 ]
  
  </summary>

  | ![마이페이지](https://github.com/user-attachments/assets/3fb20041-68d9-45fa-9b08-ef534f670874) |
  |:--:|
  | **마이페이지** |

  - **`추가 전공`** 과 **`소단위 전공`** 에 대한 정보를 수정할 수 있습니다.
  - 단, **`추가 전공`** 및 **`소단위 전공`** 변경 시 졸업요건이 변동되므로 검사를 **`재실시`** 해야 합니다.
  - 마이페이지에서 **`부족 학점`** 과 **`기이수 과목`** 을 확인할 수 있습니다.
  - **`비밀번호 재설정`** 시 기존 비밀번호를 입력하여 본인임을 확인합니다.
  <!-- - 전과 등 학과 변동 시 탈퇴 후 재가입을 통해 반영합니다. -->

</details>

<details>
  <summary>
    
  #### [ 기이수 과목 PDF 등록 ]
  
  </summary>

  | ![기이수 과목 PDF 등록](https://github.com/user-attachments/assets/487c01ec-ce14-4242-8ae4-968bb671faf6) |
  |:--:|
  | **기이수 과목 PDF 등록** |

  - **`종합정보시스템`** - **`학기별 성적조회 및 출력`** 에서 **`인쇄`** 버튼을 눌러 **`PDF로 저장`** 한 PDF를 첨부합니다.
  - **`여러 학기의 PDF`** 를 한꺼번에 등록할 수 있습니다.
  - 성적은 **`NP, F`** 과목을 필터링 하기 위해서만 사용되며 데이터베이스에 저장되지 않습니다.
  
</details>

<details>
  <summary>
    
  #### [ 기이수 과목 커스텀 ]
  
  </summary>

  | ![기이수 과목 커스텀](https://github.com/user-attachments/assets/a29d2d5c-818d-47f4-af0d-0c981b9e5890) |
  |:--:|
  | **기이수 과목 커스텀** |

  - **`현재 수강 중인 과목`** 또는 **`수강 예정 과목`** 을 반영하여 검사를 진행해 볼 수 있습니다.
  - 추가할 과목의 **`과목코드`** 로 조회한 후 과목을 선택하여 저장합니다.
  
</details>

<details>
  <summary>
    
  #### [ 졸업요건 검사 ]
  
  </summary>

  | ![졸업요건 검사](https://github.com/user-attachments/assets/2fb7c755-aa46-40fa-a22a-aca709dd8902) |
  |:--:|
  | **졸업요건 검사** |

  - 기이수 과목 PDF 등록 후 **`졸업요건 검사`** 를 진행할 수 있습니다.
  - 졸업까지 필요한 **`총 학점`** 과 **`영역별 부족한 학점`** 을 보여줍니다.
  
</details>

<br>

## 🫂 기여

<div align="center">

  | **황선규** | **황선빈** | **김중호** |
  | :------: |  :------: | :------: |
  | [<img src="https://github.com/user-attachments/assets/0e0da7f2-1b10-4fe8-8e50-088c0d043fb5" width=150> <br/> @gnawH](https://github.com/gnawH) | [<img src="https://github.com/user-attachments/assets/2acd01cd-af8d-4f35-accb-c8abec6ff5cc" width=150> <br/> @ghkdtjsqls](https://github.com/ghkdtjsqls) | [<img src="https://github.com/user-attachments/assets/b990e61b-f03a-49d3-8860-939af5369ec9" height=150 width=150> <br/> @Selene-ovo](https://github.com/Selene-ovo)|
  | **Full Stack Developer** |  **Full Stack Developer** | **FE Developer** |

</div>

<br>

## 🏆 성과



| **2024 MIRACLE 창업동아리 경진대회** |
| :------: |
| [**🥇최우수상(1등)**](https://github.com/FinishLine-CKU/FinishLine_FE/issues/51#issue-2919817249) |


### 관련 페이퍼 📰

[[이뉴스투데이 강원취재본부 최석환 기자] 가톨릭관동대, 창업기업 협의체·창업동아리경진대회 동반 개최](https://www.enewstoday.co.kr/news/articleView.html?idxno=2199031)

[[베리타스알파=김하연 기자] 가톨릭관동대, 창업기업 협의체및 창업동아리경진대회 동반 진행](https://www.veritas-a.com/news/articleView.html?idxno=529034)

<br>

## 📚 기술 스택

### FE & BE
![React](https://img.shields.io/badge/react-444444?style=for-the-badge&logo=react)
![Yarn](https://img.shields.io/badge/Yarn-2C8EBB?style=for-the-badge&logo=yarn&logoColor=white)

![Django](https://img.shields.io/badge/Django-092E20?style=for-the-badge&logo=django&logoColor=green)

![MySQL](https://img.shields.io/badge/MySQL-005C84?style=for-the-badge&logo=mysql&logoColor=white)

### Infra

![AWS](https://img.shields.io/badge/AWS-232F3E?style=for-the-badge&logo=amazonwebservices&logoColor=white)

![Nginx](https://img.shields.io/badge/Nginx-009639?style=for-the-badge&logo=nginx&logoColor=white)
![Gunicorn](https://img.shields.io/badge/Gunicorn-499848?style=for-the-badge&logo=gunicorn&logoColor=white)

### Collaboration

![Github](https://img.shields.io/badge/GitHub-100000?style=for-the-badge&logo=github&logoColor=white)
![Git](https://img.shields.io/badge/GIT-E44C30?style=for-the-badge&logo=git&logoColor=white)
![Notion](https://img.shields.io/badge/Notion-000000?style=for-the-badge&logo=notion&logoColor=white)

![Discord](https://img.shields.io/badge/Discord-5865F2?style=for-the-badge&logo=discord&logoColor=white)

### Library
- `aphrodite` : 2.4.0
- `react-icons` : 5.4.0
- `axios`: 1.7.9
- `react-chartjs-2`: 5.3.0
- `selenium` : 4.27.1
- `bs4` : 4.12.3
- `pdfplumber`: 0.11.5
- `pandas`: 2.2.3
- `openpyxl`: 3.1.5
- `django-crontab`: 0.7.1

<br>

## ⚙️ 시스템 아키텍처

<div align="center">
  <img src="https://github.com/user-attachments/assets/0b4d734d-02e0-4d94-bd8a-c333e754b411" width="800px">
</div>
