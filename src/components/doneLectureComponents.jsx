                    ...
                    {subjects && subjects.length > 0 &&
                        subjects.map((subject, index) => (
                            <tr key={subject.lecture_code}>
                                <td className={css(styles.resizeYearCell)}>{subject.year}</td>
                                <td className={css(styles.resizeYearCell)}>{subject.semester}</td>
                                <td className={css(styles.resizeYearCell)}>{subject.lecture_code}</td>
                                <td className={css(styles.resizeYearCell)} title={subject.lecture_name}>{subject.lecture_name}</td>
                                <td className={css(styles.resizeYearCell)}>{subject.lecture_type}</td>
                                <td className={css(styles.resizeYearCell)} title={subject.lecture_topic}>{subject.lecture_topic}</td>
                                <td className={css(styles.resizeYearCell)}>{subject.credit}</td>
            ...
            <table className={css(tableType === 'resize' ? Expanded ? styles.resizeTableExpend : styles.resizeTableContainer : styles.resizeTableContainer)}>
                ...
                <tbody>
                    ...
                    {sortedSubjects && sortedSubjects.length > 0 &&
                        sortedSubjects.slice(0, firstSemesterCount).map((subject) => (
                            <tr key={subject.lecture_code} className={css(subject.semester === '1' ? styles.rowColor1 : styles.rowColor2)}>
                                <td className={css(styles.resizeYearCell, subject.subjectNew ? styles.resizeYearCell : tableType === 'resize' ? styles.resizeYearCell : styles.resizeYearCell)}>
                                    {subject.year}
                                </td>
                                <td className={css(styles.resizeYearCell, subject.subjectNew ? styles.resizeYearCell : tableType === 'resize' ? styles.resizeYearCell : styles.resizeYearCell)}>
                                    {subject.semester}
                                ...
                                <td className={css(styles.resizeYearCell, subject.subjectNew ? styles.resizeYearCell : tableType === 'resize' ? styles.resizeYearCell : styles.resizeYearCell)} title={subject.lecture_name}>
                                    {subject.lecture_name}
                                </td>
                                <td className={css(styles.resizeYearCell, subject.subjectNew ? styles.resizeYearCell : tableType === 'resize' ? styles.resizeYearCell : styles.resizeYearCell)}>
                                    {subject.lecture_type}
                                </td>
                                <td className={css(styles.topicAddCell, subject.subjectNew ? styles.resizeYearCell : tableType === 'resize' ? styles.topicAddCell : styles.topicAddCell)} title={subject.lecture_topic}>
                    ...
                    {Expanded && sortedSubjects.slice(firstSemesterCount).map((subject, index) => (
                        <tr key={index + firstSemesterCount} 
                            ...
                            <td className={css(tableType === 'resize' ? styles.resizeYearCell : styles.resizeYearCell)}>{subject.year}</td>
                            <td className={css(tableType === 'resize' ? styles.resizeYearCell : styles.resizeYearCell)}>{subject.semester}</td>
                            <td className={css(tableType === 'resize' ? styles.resizeYearCell : styles.resizeYearCell)}>{subject.lecture_code}</td>
                            <td className={css(tableType === 'resize' ? styles.resizeYearCell : styles.resizeYearCell)} title={subject.lecture_name}>{subject.lecture_name}</td>
                            <td className={css(tableType === 'resize' ? styles.resizeYearCell : styles.resizeYearCell)}>{subject.lecture_type}</td>
                            ...
                            <td className={css(tableType === 'resize' ? styles.resizeYearCell : styles.resizeYearCell)}>{subject.credit}</td>
                            {tableType === 'resize' ? null :
...
const styles = StyleSheet.create({
    ...
    rowColor1: {
        backgroundColor: '#F7F2EF',  // 1학기 배경색
    },
    rowColor2: {
