    ...
    <div className={css(styles.visitorCounter)}>
      <div className={css(styles.counterContent)}>
        ...
        <p className={css(styles.counterValue)}>
          {todayVisitors.toLocaleString()}<span className={css(styles.unit)}>명</span>
        </p>
        {renderTrend()}
      </div>
    </div>
...
//         setYesterdayVisitors(yesterday);
//         setIncreaseRate(parseFloat(rate.toFixed(1)));
...
//         const counter = setInterval(() => {
//           step++;
//           setTodayVisitors(Math.floor(today * (step / steps)));
...
//         return () => clearInterval(counter);
//       } catch (error) {
//         console.error('방문자 수를 가져오는데 실패했습니다:', error);
//         setIsLoading(false);
//       }
//     };
