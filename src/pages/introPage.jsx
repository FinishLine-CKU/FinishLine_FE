...
function IntroPage() {
    ...
    const visitor = async () => {
        ...
        if (!lastVisit) {
            try {
                const response = await axios.post(`https://finishline-cku.com/user/track_visitor/`, {}, {
    ...
    const visitorcount = async () => {
        try {
            const response = await axios.get(`https://finishline-cku.com/user/track_visitor_count/`);
